...
void setup(){
  ...
  Keyboard.print("$client.DownloadFile(\"http://i.imgur.com/08bID90.jpg\", \"image.jpg\")");
  typeKey(KEY_RETURN);

  delay(4000);

  Keyboard.print("Set-ItemProperty -path 'HKCU:\\Control Panel\\Desktop\\' -name wallpaper -value \"$($userpath)\\image.jpg\"");
  typeKey(KEY_RETURN);
